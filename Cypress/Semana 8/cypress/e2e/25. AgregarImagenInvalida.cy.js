...
describe("Cypress POM Test Suite", function () {
    ...
    it('Escenario 25: HTML Valido imagen Invalida ', function () {
        ...
        cy.fixture('mock_data.json').then(data => {
            ...
            cy.get('a').then(($a) => {
                ...
                cy.get('#post-setting-codeinjection-foot > .CodeMirror > .CodeMirror-scroll').type("<img src='p' id='imagen'/>")
                
                //Then I excerpt is valido

                cy.get('p').should(($p)=>{
                    expect($p).to.contain("El codigo inyectado no es correcto");
                }).log()
