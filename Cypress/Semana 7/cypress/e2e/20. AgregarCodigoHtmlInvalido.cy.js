...
describe("Cypress POM Test Suite", function () {
    ...
    it('Escenario 20: HTML  Valido', function () {
        ...
        cy.fixture('mock_data.json').then(data => {
            ...
            cy.get('a').then(($a) => {
                ...
                cy.get('#post-setting-codeinjection-head > .CodeMirror > .CodeMirror-scroll').type("<h1>Prueba")
                
                //Then I excerpt is valido

                cy.get('P').should(($p) => {
                    expect($p).to.contain('Verifique la sintaxis');
                }).log()
